...
export default async function GolferConfirmedPage({
  ...
  return (
    <>
      <SiteHeader />
      <div className="min-h-[60vh] bg-[#FAF7F2] flex flex-col items-center justify-center px-6 py-20">
        <div className="max-w-md w-full text-center space-y-6">
          ...
          <p className="text-[#6B7770] leading-relaxed">
            ...
            {position && position <= 100 && (
              <>
                {" "}
                You&apos;re in the first{" "}
                {position <= 10 ? "10" : position <= 50 ? "50" : "100"}, early
                access guaranteed.
              </>
            )}
          </p>
          ...
          <div className="bg-white rounded-xl p-6 ring-1 ring-black/5 text-left space-y-3">
            ...
            <ul className="text-sm text-[#6B7770] space-y-2">
              <li>
                • Know a course that should partner with us? Send them to{" "}
                <strong className="text-[#1A1A1A]">
                  teeahead.com/waitlist/course
                </strong>
              </li>
              <li>
                • Share with golf friends, more local golfers = more partner
                courses
              </li>
            </ul>
          </div>
