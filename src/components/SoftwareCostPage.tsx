...
export function SoftwareCostPage({
  spotsRemaining,
  content = {},
}: SoftwareCostPageProps) {
  ...
  const [marketplaceDistribution, setMarketplaceDistribution] =
    useState<MarketplaceDistribution>("unsure");
  ...
  const savingsAsStandard = Math.max(
    0,
    totalExtraction - TEEAHEAD_PRICING.standardAnnual,
  );
          ...
          <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-12 lg:gap-14 items-start">
            ...
            <div>
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="font-mono text-xs tracking-[0.16em] uppercase text-[#E0A800] font-semibold">
                  {content["software_cost.hero_badge"] ??
              ...
              <p className="mt-4 text-base sm:text-lg text-[#F4F1EA]/78 leading-relaxed max-w-md">
                <strong className="text-[#F4F1EA]">Per year.</strong>{" "}
                Subscription, processing markup, marketplace barter, added up.
              ...
              <div className="mt-7 pt-5 border-t border-[#F4F1EA]/10">
                ...
                <ul className="flex flex-col gap-1.5">
                  {[
                    {
                      primary: fmt(annualSubscription),
                      label: "annual subscription",
                    },
                    {
                      primary: fmt(processingMarkup),
                      label: "processing markup vs 2.5% baseline",
                    },
                    {
                    ...
                    {
                      primary: `~${golferRecords.toLocaleString()}`,
                      label: "golfer records your vendor may share",
                    },
                  ].map((row) => (
                    <li
                      key={row.label}
                      className="grid grid-cols-[minmax(110px,auto)_1fr] gap-3.5 items-baseline py-1"
                    >
                      <span
              ...
              <div className="mt-7 pt-5 border-t border-[#F4F1EA]/10">
                ...
                <ul className="flex flex-col gap-1.5">
                  <li className="grid grid-cols-[minmax(110px,auto)_1fr] gap-3.5 items-baseline py-1">
                    ...
                    <span className="text-[14px] text-[#F4F1EA]">
                      Founding Partner · first year ·{" "}
                      {allClaimed
                        ? "all spots claimed"
                        : `${spotsRemaining} of 10 remaining`}
                    </span>
                  </li>
                  <li className="grid grid-cols-[minmax(110px,auto)_1fr] gap-3.5 items-baseline py-1">
                    <span
                      ...
                      style={{ fontSize: 22, fontWeight: 400 }}
                    >
                      {fmt(TEEAHEAD_PRICING.standardAnnual)}
                    </span>
                    <span className="text-[14px] text-[#F4F1EA]">
                      Standard · $349/mo flat, cancel anytime
                    </span>
                  </li>
                </ul>
                ...
                {isUnusuallyLean ? (
                  <p className="mt-4 text-[13px] text-[#F4F1EA]/70 leading-relaxed">
                    Your current setup is unusually lean. The case for TeeAhead
                    here is data ownership and the loyalty layer, not cost.
                  </p>
                ) : (
                  <p className="mt-4 text-[13px] text-[#F4F1EA] leading-relaxed">
                    ...
                    as a Founding Partner. Even at standard pricing,{" "}
                    <span className="font-semibold">
                      {fmt(savingsAsStandard)}
                    </span>
                    /yr.
                  </p>
                )}
            ...
            <div className="bg-[#F4F1EA]/[0.04] border border-[#F4F1EA]/10 rounded-2xl p-6 sm:p-7 flex flex-col gap-5">
              ...
              <Slider
                ...
                range="$0 – $1,500 · all add-ons included"
              >
                <input
                  id="monthly-sub"
                  type="range"
                  ...
                  onChange={(e) =>
                    setMonthlySubscription(Number(e.target.value))
                  }
              ...
              <Slider
                ...
                value={fmtVolume(annualCardVolume)}
                range="$100K – $5M · green fees, pro shop, F&B"
              >
                <input
                  id="card-volume"
                  type="range"
              ...
              <Slider
                label="Blended processing rate"
                value={`${paymentProcessingRate.toFixed(1)}%`}
                range="2.4 – 4.0% · baseline 2.5%"
              >
                <input
                  id="processing-rate"
                  type="range"
                  ...
                  onChange={(e) =>
                    setPaymentProcessingRate(Number(e.target.value))
                  }
                  className="w-full h-1.5 rounded-full cursor-pointer"
                  style={{ accentColor: "#E0A800" }}
              ...
              <div>
                ...
                <div
                  ...
                  role="group"
                  aria-label="Marketplace distribution"
                >
                  {(["yes", "unsure", "no"] as MarketplaceDistribution[]).map(
                    (opt) => {
                ...
                <p className="mt-1.5 font-mono text-[10px] tracking-[0.08em] text-[#F4F1EA]/40">
                  If you&apos;re on foreUP, Lightspeed, Club Caddie, or Club
                  Prophet, the answer is almost certainly yes.
                </p>
              </div>

              <Link
                href="/waitlist/course?tier=founding"
                className="mt-1 rounded-md bg-[#E0A800] px-4 py-3.5 text-sm font-bold text-[#082419] hover:bg-[#E0A800]/90 text-center"
              >
                {isUnusuallyLean
            ...
            <div className="grid sm:grid-cols-3 gap-8">
              {[
                ...
                },
                {
                  num: "5 of 6",
                  label:
                    "Major non-GolfNow vendors routing tee times to Supreme Golf marketplace by default",
                  source: "Public integration documentation, 2025–2026",
                },
                {
                  num: "Unrestricted",
                  label:
                    "Lightspeed and Club Caddie's stated rights to share aggregated golfer data with third parties",
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-[#6B7770]">
                      Parent · {vendorData.parent}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="font-mono text-[10px] tracking-[0.12em] uppercase text-[#6B7770]">
                      Typical range
                    </p>
