    ...
    <div className="rounded-xl overflow-hidden" style={{ background: '#1B4332' }}>
      ...
      <div className="px-5 pt-5 pb-4">
        <div className="flex items-center justify-between mb-1">
          <span className="text-[9px] uppercase tracking-[0.2em] text-[#aaa] font-sans">
            Round Card
          </span>
        ...
        <p className="text-[11px] text-[#aaa] font-sans mt-1">{subHeadline}</p>
      </div>
      ...
      <div className="grid grid-cols-3 border-y border-[#1d4c36]" style={{ background: '#163d2a' }}>
        <div className="px-4 py-3 text-center border-r border-[#1d4c36]">
          <p className="text-[8px] uppercase tracking-widest text-[#aaa] font-sans mb-1">Points</p>
          <p className={`text-xl font-bold font-serif ${isActive ? 'text-white' : 'text-[#8FA889]'}`}>
            {isActive ? pointsBalance.toLocaleString() : '0'}
          </p>
          <p className={`text-[8px] font-sans mt-0.5 ${isActive ? 'text-[#8FA889]' : 'text-[#8FA889]'}`}>
            {isActive ? `$${(pointsBalance / 100).toFixed(2)} val` : 'not yet'}
          </p>
        </div>
        <div className="px-4 py-3 text-center border-r border-[#1d4c36]">
          <p className="text-[8px] uppercase tracking-widest text-[#aaa] font-sans mb-1">Credit</p>
          <p className={`text-xl font-bold font-serif ${isActive && creditCents > 0 ? 'text-[#E0A800]' : 'text-[#8FA889]'}`}>
            {isActive && creditCents > 0 ? `$${(creditCents / 100).toFixed(0)}` : '—'}
          </p>
          <p className={`text-[8px] font-sans mt-0.5 ${isActive && creditCents > 0 ? 'text-[#8FA889]' : 'text-[#8FA889]'}`}>
            {isActive && creditCents > 0 ? 'ready to use' : 'earn first'}
          </p>
        </div>
        <div className="px-4 py-3 text-center">
          <p className="text-[8px] uppercase tracking-widest text-[#aaa] font-sans mb-1">Rounds</p>
          <p className={`text-xl font-bold font-serif ${isActive ? 'text-white' : 'text-[#8FA889]'}`}>
            {completedRoundsCount}
          </p>
          <p className={`text-[8px] font-sans mt-0.5 ${isActive ? 'text-[#8FA889]' : 'text-[#8FA889]'}`}>
      ...
      <div className="px-3 py-1.5" style={{ background: '#0f2d1d' }}>
        <span className="text-[8px] uppercase tracking-widest text-[#aaa] font-sans">
