          ...
          {visible.map(course => {
            ...
            return <article key={course.id} className="p-4 sm:p-5">
              ...
              <div className="mt-3 grid gap-3 text-sm xl:grid-cols-2">
                <div className="min-w-0"><p className="font-medium text-slate-700">{course.contact_name || 'Decision-maker not recorded'}</p><p className="mt-1 break-words text-xs text-slate-500">{course.contact_phone || 'Phone not recorded'}</p><p className="mt-1 break-all text-xs text-slate-500">{course.contact_email || 'Email not recorded'}</p><p className="mt-2 text-xs text-slate-600"><span className="font-medium">Software on record:</span> {course.current_software || 'Not recorded'} <span className="text-slate-400">· Confirm with course</span></p></div>
